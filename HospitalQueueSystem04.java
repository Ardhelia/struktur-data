...
//Nama :Amanda Ardhelia Rahmandi
//NIM  :243107040050
public class HospitalQueueSystem04 {
    ...
    private static void displayMenu() {
        //Menampilkan daftar opsi yang bisa dipilih user seperti tambah pasien, lihat antrian, dll.
    ...
    private static void addPatient() {
        ...
        //Menyisipkan pasien ke antrian sesuai prioritas (semakin kecil angkanya, semakin tinggi prioritas).
    ...
    private static void serveNextPatient() {
        if (patientQueue.isEmpty()) {
            ...
            //Menghapus dan mengambil pasien dengan prioritas tertinggi (paling depan).
    ...
    private static void displayQueue() {
        if (patientQueue.isEmpty()) {
            ...
            for (int i = 0; i < patientQueue.size(); i++) {
                //Menampilkan semua pasien di antrian dari prioritas tertinggi ke rendah.
    ...
    private static void updatePriority() {
        ...
        for (int i = 0; i < patientQueue.size(); i++) {
            ...
            if (p.getName().equalsIgnoreCase(name)) {
                //cek apakah nama cocok(tidak sensitif)
                ...
                //Menghapus pasien dari antrian, ubah prioritas, dan masukkan ulang ke posisi baru sesuai prioritas.
    ...
    private static void searchPatient() {
        ...
        for (Patient p : patientQueue) {
            if (p.getName().equalsIgnoreCase(name)) {
                //Mencari pasien berdasarkan nama, lalu menampilkan detail jika ditemukan.
    ...
    private static int getValidIntInput(String prompt) {
        //memastikan input angka valis dari user 
        int value;
        while (true) {
    ...
    private static int getValidIntInRange(String prompt, int min, int max) {
        //memastikan input angka dalam rentang tertentu
        int value;
        while (true) {
...
class Patient {
    ...
    public Patient(String name, int age, String condition, int priority) {
        //konstruktor untuk membuat pasien
        this.name = name;
        this.age = age;
    ...
    public void setPriority(int priority) {
        //Mengatur ulang prioritas pasien (misalnya saat diperbarui).
        this.priority = priority;
    }
}